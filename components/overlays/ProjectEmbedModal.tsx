...
export default function ProjectEmbedModal({ project, onClose }: ProjectEmbedModalProps) {
  ...
  const embedSrc = project?.embedUrl ?? project?.demoUrl;
  const isValidSrc = Boolean(embedSrc && embedSrc !== 'YOUR_DEPLOYED_URL');
          ...
          >
            ...
            <Box sx={{ position: 'relative', flex: 1, background: '#0A0A0F' }}>
              ...
              {isValidSrc ? (
                ...
                >
                  Live preview URL not set yet — add your deployed Cell Tracker URL in lib/data.ts.
                </Box>
              )}
            ...
            {isValidSrc && (
              ...
              >
                <Typography sx={{ color: '#9AA5B4', fontSize: '11px' }}>
                  Blank preview? Some sites block embedding — open in a new tab.
                </Typography>
                <Button
                  variant="outlined"
                  size="small"
